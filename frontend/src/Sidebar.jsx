    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.25)" }}>
      <div className="bg-white rounded-2xl shadow-2xl p-7 w-80 border border-gray-100">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={onConfirm} className="flex-1 py-2.5 rounded-xl bg-red-500 hover:bg-red-600 text-white text-sm font-medium transition shadow-sm">
            Log Out
          </button>
        </div>
      </div>
...
function Sidebar({ activePath }) {
  ...
  // Re-read localStorage every time the active page changes
  useEffect(() => {
    ...
    <>
      ...
      <aside className="w-60 bg-white border-r border-gray-100 shadow-sm flex flex-col h-full shrink-0">
        ...
        <div className="px-6 pt-7 pb-6 border-b border-gray-100">
          <h1 className="text-gray-800 font-bold text-base tracking-tight leading-tight">Daily Growth OS</h1>
          <p className="text-gray-400 text-xs mt-0.5 tracking-wide">Your personal OS</p>
        </div>
          ...
          >
            ...
            <div className="flex-1 text-left min-w-0">
              <p className={`text-sm font-semibold truncate leading-tight ${
                activePath === "/settings" ? "text-blue-700" : "text-gray-800"
              }`}>
                {fullName}
              </p>
              <p className={`text-[11px] truncate mt-0.5 ${
                activePath === "/settings" ? "text-blue-500" : "text-gray-400 group-hover:text-gray-500"
              }`}>
                {email}
              </p>
            </div>
