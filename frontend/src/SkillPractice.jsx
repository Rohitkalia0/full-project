...
// ─── Skill Detail Modal (full-page overlay) ───────────────────────────────────
function SkillModal({ skill, today, onClose, onSkillUpdated, showToast }) {
  const mapActivity = (a, fromYesterday) => ({
    ...
    // Only mark as carried over if it genuinely came from yesterday AND was not completed
  ...
  const toggleDone = async (id) => {
    ...
    // Optimistic update + re-sort so completed go to bottom
  ...
  const addActivity = async () => {
    ...
    try {
      ...
      if (last) {
        // New activity: never carried over, always isCarriedOver = false
        const newItem = {
          id: last.id,
          name: last.name,
          is_completed: false,
          ...
          minutes_practised: 0,
          isCarriedOver: false, // ← KEY FIX: new activities are never carried over
        };
        // Prepend new activity at top of incomplete section
            ...
            <div className="flex-1 min-w-0">
              {editingTitle ? (
                <input
                  ...
                  onKeyDown={e => e.key === "Enter" && saveSkillName()}
              ...
              <div className="flex items-center gap-3 mt-1">
                ...
                {allDone && (
                  <span className="flex items-center gap-1 text-xs font-semibold text-blue-600">
                    <BadgeCheckIcon /> Completed!
                  </span>
                )}
                {isFromYesterday && !allDone && (
                  <span className="text-[10px] font-medium text-amber-500">Carried over from yesterday</span>
                )}
              </div>
            </div>
            <button
          ...
          <div className="px-6 py-4 border-b border-gray-100">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm text-gray-700 placeholder-gray-300 outline-none focus:border-blue-300 focus:bg-white transition"
              />
              <button
                onClick={addActivity}
              ...
              >
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className={`min-w-[20px] min-h-[20px] w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all shrink-0 ${
                  ...
                  {editingActivityId === a.id ? (
                    ...
                    <span className={`flex-1 text-sm ${a.is_completed ? "line-through text-gray-400" : "text-gray-700"}`}>
                      {a.name}
                      {a.isCarriedOver && (
                        <span className="ml-2 text-[10px] text-amber-500 font-medium">carried over</span>
                      )}
                    </span>
                  )}
                </div>
                ...
                <div className="flex items-center gap-1 mt-2.5 pt-2.5 border-t border-gray-100">
                  <Tooltip text="Edit">
                    <button onClick={() => startEdit(a)} className="p-1.5 rounded-lg text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition">
                      <PencilIcon />
                    </button>
                  </Tooltip>
                  <Tooltip text="Priority">
                    <button onClick={() => togglePriority(a.id)} className={`p-1.5 rounded-lg transition ${a.is_priority ? "text-amber-500 bg-amber-50" : "text-gray-400 hover:text-amber-400 hover:bg-amber-50"}`}>
                      <StarIcon filled={a.is_priority} />
                    </button>
                  </Tooltip>
                  <Tooltip text="Protect habit">
                    <button onClick={() => toggleHabit(a.id)} className={`p-1.5 rounded-lg transition ${a.is_habit_to_protect ? "text-blue-600 bg-blue-50" : "text-gray-400 hover:text-blue-500 hover:bg-blue-50"}`}>
                      <ShieldIcon filled={a.is_habit_to_protect} />
                    </button>
                  </Tooltip>
                  <Tooltip text="Delete">
                    <button onClick={() => setDeleteTarget({ id: a.id })} className="p-1.5 rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 transition">
                      <TrashIcon />
                    </button>
                  </Tooltip>
                </div>
...
function SkillCard({ skill, today, onClick }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full text-left bg-white rounded-2xl border shadow-sm p-5 transition-all hover:shadow-md hover:-translate-y-0.5 active:scale-[0.98] ${
        allDone ? "border-blue-200" : isFromYesterday ? "border-amber-200" : "border-gray-100"
      }`}
    >
      {/* Title row */}
      <div className="flex items-start justify-between gap-2 mb-3">
        <h3 className={`text-sm font-semibold truncate leading-tight ${allDone ? "text-blue-700" : "text-gray-800"}`}>
          {skill.name}
        </h3>
        {allDone && (
          <span className="shrink-0 text-blue-500"><BadgeCheckIcon /></span>
        )}
      </div>
      ...
      <div className="mb-2">
        <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
          <div
            className={`h-full rounded-full transition-all ${allDone ? "bg-blue-500" : "bg-blue-400"}`}
            style={{ width: `${pct}%` }}
          />
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        {isFromYesterday && !allDone && (
          <span className="text-[10px] font-medium text-amber-500">carried over</span>
        )}
        {allDone && (
          <span className="text-[10px] font-semibold text-blue-600">Complete!</span>
        )}
      </div>
    </button>
  );
...
function SkillPractice() {
  ...
  const addSkill = async () => {
    ...
    try {
      ...
      // Prepend new skill at the front
  ...
  const handleSkillUpdated = (id, patch) => {
    ...
    // Also update selectedSkill if open
